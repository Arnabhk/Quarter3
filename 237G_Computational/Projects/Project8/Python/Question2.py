...
#Helps price call option on discount bond for CIR (underlying priced using monte carlo)
def price_call_discount_bond_mc(r0, sigma, k, ravg, fv, time, step_size, no_of_sim, expiry, strike):
    ...
    call_prices = [price_call_discount_iter(no_of_steps, r0, k, ravg, step_size,sigma, time, expiry, fv, no_of_sim, strike, False)
                   for count in range(0,no_of_sim)]
    return statistics.mean(call_prices)


#Helps price call option on discount bond for CIR (underlying priced explicitly)
def price_call_discount_bond(r0, sigma, k, ravg, fv, time, step_size, no_of_sim, expiry, strike):
    no_of_steps = int(expiry/step_size)

    call_prices = [price_call_discount_iter(no_of_steps, r0, k, ravg, step_size, sigma, time, expiry, fv, no_of_sim, strike, True)
        for count in range(0, no_of_sim)]
    return statistics.mean(call_prices)
...
def price_call_discount_iter(no_of_steps, r0, k, ravg, step_size,sigma, time, expiry, fv, no_of_sim, strike, is_explicit):
    ...
    if is_explicit:
        ...
        price = Question1.price_discount_bond(r_path[no_of_steps - 1], sigma, k, ravg, fv, time - expiry,
...
#Helps calculate the price on bond using implicit finite differential method
